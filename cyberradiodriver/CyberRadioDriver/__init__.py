...
#    <tr><td>\link CyberRadioDriver::radios::internal::ndr374::ndr374 NDR374 \endlink</td><td>"ndr374"</td></tr>
...
if sys.platform != "win32":
    ...
    def getInterfaceAddresses(ifname, getMTU=False):
        if sys.version_info >= (3,):
            def IFNAME_CVT(ifname):
                return ifname.encode("ascii")
            def CHAR_CVT(char):
                return char
        else:
            def IFNAME_CVT(ifname):
        ...
        try:
            ...
            ifr = struct.pack('256s', IFNAME_CVT(ifname)[:15])
            info = fcntl.ioctl(s.fileno(),
                               0x8927, # SIOCGIFHWADDR
                               ifr)
            mac = ''.join(['%02x:' % CHAR_CVT(char) for char in info[18:24]])[:-1]
